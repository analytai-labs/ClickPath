...
import {
  ...
  type PharmaThemeTokens,
  resolvePharmaTheme,
} from "@/lib/templates/definitions/pharma-product";

import { PharmaProductCarousel } from "./carousel";
...
type Props = {
  ...
  /** Styling variant id, stored as `theme.preset`. */
...
export function PharmaProductRenderer({
  ...
  const t = resolvePharmaTheme(variantId);
  const documents = data.documents.filter((d) => d.imageUrl);
  const hasContact = Boolean(data.contact.name || data.contact.whatsapp || data.contact.email);
  ...
  return (
    ...
    >
      {/* Hero */}
      <div style={{ background: t.heroBackground ?? t.accentColor }} className="px-5 pb-6 pt-8">
        <div
        ...
        >
          ...
          <h1 className="mt-1 text-2xl font-bold leading-snug" style={{ color: t.accentTextColor }}>
      ...
      <div className="flex-1 px-5 pb-12 pt-5">
        <div className="mx-auto max-w-md space-y-5">
          ...
          {data.productImages.length > 0 && (
            <div className={ENTER} style={{ animationDelay: "70ms", animationDuration: "450ms" }}>
              ...
              <div className="grid grid-cols-2 gap-3">
                {documents.map((doc, i) => (
                  ...
                  >
                    ...
                    <img src={doc.imageUrl} alt={doc.name} className="h-24 w-full object-cover" />
                    <p
          ...
          {hasContact && (
            <Section title="Contact" t={t} delay={350}>
              <div className="space-y-2">
                {data.contact.name && <p className="text-sm font-medium">{data.contact.name}</p>}
...
function ContactButton({ href, label, t }: { href: string; label: string; t: PharmaThemeTokens }) {
  return (
    <a
      href={href}
