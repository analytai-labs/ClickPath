...
type RecordTemplatePageViewOptions = {
  headers: Headers;
  templatePageId: number;
  /** Page owner whose monthly event quota the view consumes. */
  ownerId: string;
  ip: string;
  country: string;
  city: string;
 ...
 */
async function recordUniqueView(ipHash: string, templatePageId: number) {
  await prisma.uniqueTemplatePageView
    .create({
      data: { ipHash, templatePageId },
    })
    .catch(() => {
...
/**
 * Records a template-page view. Mirrors recordClick: bot-filtered, IP-hashed, and
 ...
 */
export async function recordTemplatePageView(opts: RecordTemplatePageViewOptions): Promise<void> {
  const { headers, templatePageId, ownerId, ip, country, city } = opts;
  ...
  await Promise.all([
    ...
    recordUniqueView(ipHash, templatePageId),
  ]);
}
